...
#. Check if service is enabled, if not, disable it::
...
#. On iDrac, use the filter functionality found in the storage overview to find the pysical location of the drive. **Storage -> Physical Disks -> Filter Drives/Options**
...
If that doesn't solve the problem. Dig a rabbit hole. Start with the log of the primary OSD
